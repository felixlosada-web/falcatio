...
export default function Header() {
  return (
    <header className="fixed top-0 left-0 w-full z-50 bg-[#F8F6F2]/95 border-b border-black/5">
      <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-24 py-6 flex items-center justify-between gap-10">
        <Link
          href="/"
          className="flex flex-col items-center gap-3 group shrink-0 w-[280px]"
        >
          <img
            src="/logo-falcatio.png"
            alt="Falcatio"
            className="w-12 h-auto opacity-90 transition-opacity group-hover:opacity-100"
          />

          <div className="flex flex-col items-center text-center">
            <span className="text-[14px] tracking-[0.16em] text-black uppercase font-medium leading-none">
              Falcatio Librorum
            </span>

            <span className="text-[8px] tracking-[0.12em] text-black/40 uppercase mt-2">
              Curaduría Bibliográfica Privada
            </span>
          </div>
        </Link>

        <nav className="flex flex-wrap justify-end gap-x-5 gap-y-3 text-[10px] tracking-[0.15em] uppercase">
